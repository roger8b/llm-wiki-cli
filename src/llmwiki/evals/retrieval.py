"""Retrieval eval harness: golden set, metrics, search/ask eval, report (#349).

Lives in the package (not in ``scripts/``) so tests and the CLI harness import
it the same way — a module loaded by file path makes dataclasses resolve their
defining module through ``sys.modules`` (#366).
"""
...
def _rank(
    conn, cfg, query: str, mode: str, limit: int, embedder, store
) -> list[str] | None:
    ...
    return [
        ...
        for h in hybrid_search(
            conn, query, limit=limit, embedder=embedder, store=store
        )
    ]


def run_search_eval(
    conn, cfg, cases: list[GoldenCase], *, limit: int = 10
) -> dict:
